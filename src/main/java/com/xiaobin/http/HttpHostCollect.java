...
public class HttpHostCollect {
    ...
    static void start(){
        ...
        if(dao.findOne("select * from constant where type = ?", CONST_TYPE).isEmpty()){
            Map<String, Object> map = dao.findOne("select min(create_time) as create_time from http_host_detail");
            ...
            if(dao.exec("insert into constant(type, value) values(?, ?)", CONST_TYPE, map.get("create_time")) == 0){
    ...
    private void addNewHost(){
        ...
        Map<String, Object> map = dao.findOne("select * from constant where type = ?", CONST_TYPE);
        ...
        List<Map<String, Object>> list = dao.findList(sql, map.get("value"));
        if(list.size() > 0){
            emptyCount = 0;
            ...
            if(!paramsList.isEmpty()){
                for(Map<String, Object> tmp: paramsList){
                    ...
                    if(!dao.exist("select 1 from http_host where host = ?", host)){
                        dao.exec("insert into http_host(id, host, protocol, port, url_state, create_time) values(?,?,?,?,?,?)"
                                , UUID.randomUUID().toString().replaceAll("-", "")
                                , host, tmp.get("protocol"), -1, -1, System.currentTimeMillis());
                    }
                }
            }
            dao.exec("update constant set value = ? where type = ?", list.get(list.size() - 1).get("create_time"), CONST_TYPE);
        }else{
            emptyCount++;
            detailEmpty = true;
        }
                ...
                while(!list.isEmpty()){
                    for(Map<String, Object> map: list){
                        ...
                        try {
                            ...
                            dao.exec("update http_host set ip = ?, state = ? where id = ?", address, status, id);
                        }
                    }
                    list = find();
                }
    ...
    private List<Map<String, Object>> find(){
        return dao.findList("select * from http_host where state = 0 limit 0, 500");
    }
}
